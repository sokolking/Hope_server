...
{
    private void ClampUnitsToActiveZoneIfNeeded(List<PlayerTurnResultDto>? results = null)
    {
        EnsureActiveZoneInitialized();
        int minC = _activeMinCol;
        int maxC = _activeMaxCol;
        int minR = _activeMinRow;
        int maxR = _activeMaxRow;

        var occupied = new HashSet<(int col, int row)>();
        foreach (var u in Units.Values)
        {
            if (u.CurrentHp > 0 && u.Col >= minC && u.Col <= maxC && u.Row >= minR && u.Row <= maxR)
        ...
        foreach (var uid in Units.Keys.ToList())
        {
            ...
            int uc = unit.Col;
            int ur = unit.Row;
            if (uc >= minC && uc <= maxC && ur >= minR && ur <= maxR)
                continue;

            if (!TryFindNearestFreeHexInRectangle(uc, ur, minC, maxC, minR, maxR, occupied, out int nc, out int nr))
                continue;
    ...
    private HexPositionDto[]? ApplyZoneShrinkIfNeeded(int resolvedRoundIndex, List<MapUpdateDto> mapUpdates, List<PlayerTurnResultDto> results)
    {
        ...
        foreach (var cell in removedCells)
        {
            if (_obstacleTags.TryGetValue(cell, out var tag) && IsWallObstacleTag(tag))
                mapUpdates.Add(new MapUpdateDto { Tick = ZoneShrinkMapTick, Hex = HexTagFromColRow(cell.col, cell.row), NewState = CellObjectState.None });
        ...
        ClampUnitsToActiveZoneIfNeeded(results);
