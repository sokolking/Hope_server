...
| Метод | Путь | Описание |
|-------|------|----------|
| POST | `/api/battle/join` | Вход в бой или постановка в очередь. Тело: `hex` (формат `A0..Z99`), опционально `solo` (одиночный бой с мобом). Учитывается профиль и экипировка из БД. |
