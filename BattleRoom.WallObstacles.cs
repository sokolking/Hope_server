...
public partial class BattleRoom
{
        ...
        List<MapUpdateDto> mapUpdates)
    {
        ...
        if (hp <= 0)
        {
            ...
            mapUpdates.Add(new MapUpdateDto { Tick = tick, Hex = HexTagFromColRow(wc.col, wc.row), NewState = CellObjectState.None });
            return;
        }
        ...
        mapUpdates.Add(new MapUpdateDto { Tick = tick, Hex = HexTagFromColRow(wc.col, wc.row), NewState = newState });
    }
}
