...
builder.Services.AddSingleton<BattleServerLogDatabase>();
builder.Services.AddSingleton<BattleLogStore>(sp =>
{
    var logDb = sp.GetRequiredService<BattleServerLogDatabase>();
    return new BattleLogStore(entry =>
    {
        try
        {
            logDb.Save(entry);
        ...
        {
            // Keep console/memory log flow running even if DB sink fails.
        }
    });
});
...
app.MapPut("/api/db/users", (BattleUserDatabase users, UserUpdateRequest? body) =>
{
    if (body == null)
